...
export default function CreateCampaignPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setClientBusinessDescription(data.clientBusinessDescription ?? "");
    }
  }, []);
  ...
  const handleSubmit = () => {
    ...
    if (rewardStyle === "paid") {
      ...
      if (!clientAddress.trim()) {
        alert("本社所在地を入力してください");
        return;
      }
      if (!clientBusinessDescription.trim()) {
        alert("事業内容を入力してください");
        return;
      }
      ...
      sessionStorage.setItem("campaign-step1", JSON.stringify({
        ...
        clientName, clientAddress, clientWebsite, clientBusinessDescription,
